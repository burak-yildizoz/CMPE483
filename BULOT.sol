...
contract BULOT
{
    ...
    mapping(uint => mapping(uint => bytes32)) hashes;     // database for hash of random numbers stored:   first index: lottery_no, second index: ticket_no, --employs cleaning
    mapping(uint => bytes32) lotteryRandom;               // list for the random numbers calculated for each week's lottery
    mapping(uint => mapping(uint=> address)) ticketowner; // (lotteryno, ticketno) => owner   // to authenticate withdraw, --employs cleaning
    mapping(address => uint) lastBoughtTicket;            // owner => owner's last bought ticket no.  Set by buyTicket, read by getLastBoughtTicketNo
    mapping(uint => mapping(uint => bool)) notrevealed;   // (lotteryno, ticketno) => redeemable (those who do not reveal will not get a prize), --employs cleaning
    ...
    function revealRndNumber        (uint ticket_no, uint rnd_number)    public
    {
        ...
        delete hashes[last_lottery_no][ticket_no];                                                                              //The hash will not be necessary anymore
        delete notrevealed[last_lottery_no][ticket_no];                                                                         //Update the state of having revealed
    ...
    function withdrawTicketPrize    (uint lottery_no, uint ticket_no)   public
    {
        ...
        require(ticketowner[lottery_no][ticket_no] == msg.sender, "Only the ticket owner can claim reward, and only once");      //Authenticate that entity willing to receive the price is who bought this ticket
        ...
        delete ticketowner[lottery_no][ticket_no];                                                                               //Disable ability of this ticket to do another withdraw and avoid reentrancy attacks
